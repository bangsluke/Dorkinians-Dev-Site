...
function init() {
	...
	// Step 0.
	console.log(
		"%c" + "> 0. init() called. Code started for each of the three sub processes.",
		"background-color: #1C8841; color: white; padding: 0.5em 0em; font-weight: bold;"
...
function openNav() {
	...
	document.getElementsByTagName("side-menu-component")[0].shadowRoot.getElementById("side-menu").style.width =
		sideMenuWidth; // Increase the width of the side-menu to make it visible.
...
function showSideMenuInfo(results) {
	...
	document
		.getElementsByTagName("side-menu-component")[0]
		.shadowRoot.getElementById("side-menu-site-details-version-number-text").innerHTML =
		displaySiteDetailsArrayOfObjects[0]["Version Number"]; // Add the text to the side menu.
	document
		.getElementsByTagName("side-menu-component")[0]
		.shadowRoot.getElementById("side-menu-site-details-current-season-text").innerHTML =
		displaySiteDetailsArrayOfObjects[0]["Current Season"]; // Add the text to the side menu.
	document
		.getElementsByTagName("side-menu-component")[0]
		.shadowRoot.getElementById("side-menu-site-details-last-updated-stats-text").innerHTML =
		displaySiteDetailsArrayOfObjects[0]["Last Updated Stats"]; // Add the text to the side menu.
	document
		.getElementsByTagName("side-menu-component")[0]
		.shadowRoot.getElementById("side-menu-site-details-page-details-last-refereshed-text").innerHTML =
...
function getPlayerDropdownInfo() {
	...
	const displayAllowedPlayersArrayOfObjects = displayAllTimeStatsArrayOfObjects.filter(
		...
		for (let j = 0; j < statsArray.length; j++) {
			...
			try {
				...
				document.getElementById(
					"player-stats-" + playerStatsSectionArray[i] + "-stat-text-" + statsArray[j]
	...
	for (let i = 0; i < statsArray.length; i++) {
		...
		try {
			...
			if (StatFormat == "Integer") {
				...
				var displayText = Number(filteredArrayOfObjects[0][statsArray[i]]).toLocaleString(
					"en-UK"
				); // Use a dynamic [statArray[i]] key. Convert the stat to a number and then add a comma by using the "toLocaleString" method.
...
const showTOTWPlayerInfo = function () {
	...
	for (let i = 0; i < statArray.length; i++) {
		...
		if (statArray[i] == "G") {
			...
			document.getElementById("totw-player-info-box-" + statArray[i] + "-row").classList.add(
				"hidden"
			);
		} else {
			...
			document.getElementById("totw-player-info-box-" + statArray[i]).innerHTML =
				playerMatchDetailsData[0][statArray[i]];
			...
			switch (statArray[i]) {
				...
				case "C":
					if (
						...
						document.getElementById("totw-player-info-box-C-row").classList.add(
							"hidden"
						);
						document.getElementById(
							"totw-player-info-box-CLS-row"
						).classList.remove("hidden");
						document.getElementById("totw-player-info-box-CLS").innerHTML = 1;
						document.getElementById("totw-player-info-box-CLS-points").innerHTML =
							1 * multiplierValue;
					} else {
						// Change the multiplier value based on the class of the player.
						switch (playerClass) {
							case "GK":
								document.getElementById(
									...
									playerMatchDetailsData[0]["C"] * -0.5
								); // Round the number of goals conceded points to be an integer.
								break;
							case "DEF":
								document.getElementById(
									"totw-player-info-box-C-points"
...
function showComparisonStatData() {
	...
	for (let i = 0; i < statsArray.length; i++) {
		...
		try {
			...
			document.getElementById("comparison-" + statsArray[i] + "-short-text").innerHTML =
				statObject[statsArray[i]].shortText; // Populate the short text element dynamically.
...
function loadInComparisonStatNumbers(statName, player1Name, player2Name, fillBarsBoolean) {
	...
	if (statValueArray[1] + statValueArray[2] === 0) {
		BarElement = document.getElementById("comparison-" + statName + "-player-1-bar-container").style.width =
			0 + "%";
		BarElement = document.getElementById("comparison-" + statName + "-player-2-bar-container").style.width =
			0 + "%";
	} else {
